...
saveRDS(genomeMotifGR,"/Users/semple/Documents/MeisterLab/GenomeVer/sequence/c_elegans.PRJNA13758.WS250.genomic.CGGC_motifs.RDS")
```

Once the RDS is saved it can be loaded for all future analyses.

```{r}
genomeMotifGR<-readRDS("/Users/semple/Documents/MeisterLab/GenomeVer/sequence/c_elegans.PRJNA13758.WS250.genomic.CGGC_motifs.RDS")
```
...
```{r}
methMat<-methMatrix::combineCGandGCmatrices(matCG,matGC,regionGR,genomeMotifGR)
methMat[2:5,10:18]
...
matTableFile<-system.file("extdata", "csv/MatrixLog_ampTSS.csv", package="methMatrix", mustWork=TRUE)
matTable<-read.csv(matTableFile,stringsAsFactors=F)

# modify teh matTable path to data
matTable$filename <-system.file("extdata", matTable[,"filename"], package="methMatrix", mustWork=TRUE)
...
tssFile<-system.file("extdata","genome/ampliconMaxTSSgr.RDS",package="methMatrix",mustWork=TRUE)
tssWin<-readRDS(tssFile)
names(mcols(tssWin))<-"ID"
...
matTableFile<-system.file("extdata", "csv/MatrixLog_relCoord_ampTSS.csv", package="methMatrix", mustWork=TRUE)
matTable<-read.csv(matTableFile,stringsAsFactors=F)
matTable$filename <-system.file("extdata", matTable[,"filename"], package="methMatrix", mustWork=TRUE)

allSampleRelCoordMats<-getRelativeCoordMats(matList=matTable, regionGRs=tssWin, regionType=regionType, anchorCoord=winSize/2)
...
plotAllMatrices(allSampleRelCoordMats, samples, regionGRs=tssWinRelCoord,  
                featureGRs=TSSrelCoord, featureLabel="TSS", regionType=regionType,
